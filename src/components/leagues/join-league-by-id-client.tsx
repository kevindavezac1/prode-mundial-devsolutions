...
export function JoinLeagueByIdClient({ league, isLoggedIn }: Props) {
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const permanentPath = `/join/liga/${league.id}`;
  ...
  function handleGoogle() {
    startTransition(async () => {
      await loginWithGoogle(permanentPath);
    });
  }
      ...
      >
        <div className="w-full max-w-sm space-y-5">
          ...
          <div className="space-y-3">
            ...
            >
              {isPending ? "REDIRIGIENDO..." : <><GoogleIcon />ENTRAR CON GOOGLE PARA UNIRME</>}
            </button>

            <p className="text-center text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>
              También podés{" "}
              <Link
                href={`/login?redirect=${permanentPath}`}
            ...
            <p className="text-center text-xs" style={{ color: "rgba(255,255,255,0.25)" }}>
              ¿No tenés cuenta?{" "}
              <Link
                href={`/register?redirect=${permanentPath}`}
                className="underline underline-offset-2"
